defmodule Storex.SalesTest do
  ...
  describe "orders" do
    ...
    test "process_order/3 creates an order" do
      ...
      {:ok, order} =
        Sales.process_order(user, cart, %{address: "Test Street, 25"})
    ...
    test "list_orders/0 returns all created orders" do
      ...
      {:ok, order1} =
        Sales.process_order(user, cart, %{address: "Street One, 25"})

      {:ok, order2} =
        Sales.process_order(user, cart, %{address: "Street Two, 25"})

      {:ok, order3} =
        Sales.process_order(user, cart, %{address: "Street Three, 25"})
